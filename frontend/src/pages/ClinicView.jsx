...
export default function ClinicView() {
  ...
  // ── Clinic body (shared between split and single layout) ──────────────
  const clinicBody = (
    <>
      {/* ── HERO ── */}
      <div style={{
  ...
  // ── Fixed background layers (rendered in both modes) ─────────────────
  const backgrounds = (
    <>
      <div style={{
  ...
  // ── Split layout when AI panel is open ───────────────────────────────
  if (aiOpen) {
    return (
      <div className="h-screen flex flex-col overflow-hidden" style={{ background: '#F5F0EB' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
            {/* Left: scrollable clinic content */}
            <div style={{ flex: 1, overflowY: 'auto', minWidth: 0 }}>
              {clinicBody}
            </div>
            {/* Right: AI panel — full height, fixed width */}
            <div style={{
              width: 440,
              flexShrink: 0,
              display: 'flex',
  ...
  // ── Single-column layout (AI panel closed) ───────────────────────────
  return (
    <div className="min-h-screen" style={{ background: '#F5F0EB' }}>
      {backgrounds}
