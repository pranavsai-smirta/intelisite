export default function KpiCard({ label, value, unit, delta, higherIsBetter, subtitle, showTrendLink, heroAccent }) {
  ...
  return (
    ...
    >
      <div className="text-lg font-semibold text-[#64748B] uppercase tracking-wider">{label}</div>
      <div className="flex items-end gap-2">
        <span className="text-3xl font-bold text-[#1A1A2E]">
          {value !== null && value !== undefined ? value : '—'}
        </span>
        {unit && <span className="text-[#64748B] text-base mb-1">{unit}</span>}
      </div>
      {hasDelta && (
        <div className={`text-base font-medium ${deltaColor} flex items-center gap-1`}>
          <span>{deltaArrow}</span>
          <span>{Math.abs(delta).toFixed(1)} MoM</span>
        </div>
      )}
      {subtitle && <div className="text-base text-[#94A3B8] mt-1">{subtitle}</div>}
      {showTrendLink && (
        <button
          onClick={() => document.getElementById('trend-charts')?.scrollIntoView({ behavior: 'smooth' })}
          className="mt-auto w-full text-sm font-semibold text-left px-3 py-2 rounded-lg transition-all"
          style={{
        ...
        >
          {'↗'} Click here to view 6 month trend
        </button>
      )}
    </div>
  )
